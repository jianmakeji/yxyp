    ...
    return {
        ...
        createUploader:function(params){
            var uploader=new plupload.Uploader({
                runtimes:"html5,flash",					//用来指定上传方式，指定多个上传方式请使用逗号隔开。
                multi_selection:params.multiSelection,	//是否可以在文件浏览对话框中选择多个文件，true为可以，false为不可以。
                max_file_size:params.maxSize,			
                browse_button:params.uploadBtn,			//触发文件选择对话框的DOM元素，当点击该元素后便后弹出文件选择对话框。
                container:params.uploadContainer,		//用来指定Plupload所创建的html结构的父容器，默认为前面指定的browse_button的父元素。
                multipart_params:params.multipartParams,	//上传时的附加参数，以键/值对的形式传入，服务器端可是使用$_POST来获取这些参数.
                url:document.getElementById("baseUrl").getAttribute("href")+config.uploader.url,	//服务器端接收和处理上传文件的脚本地址，可以是相对路径(相对于当前调用Plupload的文档)，也可以是绝对路径
                flash_swf_url:document.getElementById("baseUrl").getAttribute("href")+config.uploader.swfUrl,	//flash上传组件的url地址，如果是相对路径，则相对的是调用Plupload的html文档。
                filters : [								//可以使用该参数来限制上传文件的类型，大小等，该参数以对象的形式传入
                    {title : "Media files", extensions : params.filter}
                ]
            });

            //初始化		当Plupload初始化完成后触发
            uploader.init();	

            //文件添加事件		当文件添加到上传队列后触发
            ...
            //文件上传进度条事件		会在文件上传过程中不断触发，可以用此事件来显示上传进度

...
public class ReviewServiceImpl implements ReviewService {
	...
	public PagingModel getReviewListByUserId(int userId, int scoreSign, int round, int offset, int limit) {
		...
		pagingModel.setList(reviewDaoImpl.getReviewListByUserId(userId, scoreSign, round, offset, limit));
		pagingModel.setCount(reviewDaoImpl.getCountReviewByUserId(userId, scoreSign, round));
		return pagingModel;
	}
	...
	public int bindProductAndRound(int productionId, int round) {
		...
		if (roundJudge.getJudge() != null){
			...
			if (status > 2){
				...
				List<Review> reviewList = reviewDaoImpl.getReviewByProductIdAndRound(productionId, round);
				if (reviewList.size() > 0){
					return ResponseCodeUtil.BIND_JUDGE_REPEAT;
				}
				else{
